...
Allowed mutation operations are `insert_event`, `split_event`, `merge_events`, `retarget_pitch`, and `shift_boundary`. Every mutation must cite at least one non-baseline artifact from a targeted specialist tool and include a precise reason. Use the exact baseline event IDs. Set `state` to `accepted` only when every proposed mutation has decisive local evidence; for a retrigger this means both a targeted onset/energy observation and a targeted voiced stable-pitch observation support the same window. Otherwise set `state` to `uncertain` and keep the baseline. A `split_event` is an articulation decision: the first segment is the original sustain up to the measured boundary, and the second segment is a fresh same-pitch attack/retrigger. Use `split_event` only when local onset/energy evidence and voiced stable-pitch evidence support a new attack; otherwise preserve one continuous event. Prefer no mutation over an unsupported mutation. Never return a replacement melody array.

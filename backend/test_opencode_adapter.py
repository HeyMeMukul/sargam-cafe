...
async def fake_runner(cmd, log_callback, cost_tracker=None):
    return __import__('json').dumps(payload)

async def fake_log(message):
    pass

result = asyncio.run(run_opencode_micro_agent('/tmp/a.mp3', 2.0, [{'start':0,'end':1,'midi':60}], fake_runner, fake_log, 'model', 2))
